...
## v2.1.2
- 设置页新增自动填充（预留）开关；导入/导出增强校验（空数据提示、格式错误提示、版本校验），导出支持一键复制并可自动清空剪贴板，导入支持字段缺失容错并提示兼容旧版格式。
- 导入/导出入口移入设置页（支持导出 JSON + 导入覆盖，提示“导出后密码为明文，请妥善保存”，导出包含分组与 schema）。
...
## v2.2.2
- 安全体验：自动锁定延迟提示更清晰；敏感信息保护提示更明确。

## v2.3.0
- 导入/导出：支持加密导出（自定义口令）与导入预览。

## v2.3.1
- 自动填充：域名匹配更智能（精确>子域名>模糊关键词），最近7天使用过的账号优先展示，增加「最近使用」提示。

## v2.2.1
- 自动填充：设置页支持跳转系统开关；列表增加来源/账号提示，手动选择更清晰。
- 导入/导出：导出增加 schema 字段；导入字段缺失容错。

## v2.2.0
- 自动填充：系统级服务可用，支持域名/包名匹配、记住上次选择、默认项优先与手动选择提示。
- 导入/导出：支持版本号与分组导出，兼容旧版格式，导出一键复制并自动清空剪贴板。
- 安全优化：后台自动锁定提示更清晰；生物识别提示更清晰；敏感信息隐藏文案更明确且不可复制。


## v2.1.3
- 自动填充排序优化：最近使用优先（在默认项之后）。
- 自动填充匹配优化：支持包名/域名双重匹配，优先上次选择（webDomain 优先），子域名可用；无匹配时进入手动选择并提示记住，列表增加来源提示与账号区分。
- App 内入口：详情页新增“设为自动填充默认项”，设置后提示优先自动填充。
- 自动填充：新增系统级 Autofill Service，支持浏览器/常见 App，手动选择并记住上次选择；设置页支持跳转系统开关。
- 自动填充默认项设置：列表长按可设为该域名默认填充项。
- 后台自动锁定策略优化：支持更短锁定延迟（立即/30s/1min）设置，回到前台会取消等待锁定。
- 生物识别开关与应用锁关联：仅在设备可用且已开启应用锁时可启用，并提供引导提示。
- 敏感信息隐藏强化：账号/网址/可复制字段在开启隐藏时遮罩、文案提示且禁止复制。
