...
- 已开始第三轮交互优化：新增/编辑凭据、创建分组后尽量走局部状态更新，减少整页 reload
- 已开始第四轮详情优化：为已查看/刚编辑的凭据详情增加内存小缓存，减少重复读库与解密
- 已开始第五轮状态协同优化：切分组/搜索时尽量保留仍然可见的详情，避免无谓关闭与重读
- 已开始第六轮状态工厂轻量优化：减少重复分组计算，并统一搜索词标准化处理
- 已开始第七轮点击路径优化：列表项点击时若详情已命中缓存则立即展示，减少打开迟滞
- 已修复自定义字段重新打开后显示密文的问题：读取详情时恢复自定义字段标题与内容解密
...
- 已开始第十七轮 120Hz 掉帧优化：详情面板与列表状态解耦，减少选中变化触发列表重算
